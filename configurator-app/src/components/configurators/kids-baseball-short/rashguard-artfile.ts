/**
 * Actual-size art-file export for the kids baseball short.
 *
...
// Pieces to export, in order: both fronts, then both backs. All four panels
// are printable zones and take artwork.
...
export async function generateRashguardArtFile(
  ...
  const opts: Required<Omit<ArtFileOptions, 'fileName'>> & { fileName: string } = {
    ...
    fileName: input.options?.fileName ?? 'kids-baseball-short-art-file.pdf',
  };

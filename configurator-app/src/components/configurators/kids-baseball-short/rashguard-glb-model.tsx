...
// This model has no separate lining meshes — CLO bundled each panel's inner
// surface into the same mesh as its outer surface — so this never matches.
// Kept so the shared plumbing stays uniform across garments.
function liningPartForMesh(mesh: Mesh): RashguardPart | null {
  ...
  if (n.includes('right front')) return 'rightFrontLeg';
  if (n.includes('left front')) return 'leftFrontLeg';
  if (n.includes('right back')) return 'rightBackLeg';
  if (n.includes('left back')) return 'leftBackLeg';
  return null;
}
...
function shouldShowControlsForCamera(
  ...
  if (target === 'rightBackLeg' || target === 'leftBackLeg') {
...
async function drawFixedDsplnMarks(
  ...
  _target: RashguardArtworkTarget,
) {
  // No auto-placed fixed logo on the baseball short (same as the grappling
  // short). This model has no branding mesh either — where DSPLN branding goes
  // on this garment is still an open product decision.
}
...
export const RashguardGlbModel = memo(() => {
  ...
  const cloned = useMemo(() => {
    ...
    // exports) while the camera is Y-up. Auto-detect and bake a −90° X rotation
    // into the body geometry so it stands upright. (No-op for this export,
    // which CLO already wrote Y-up.)
    ...
    // No stitch graft here: CLO exported this model's topstitching as real mesh
    // inside the single GLB (the 44 Topstitch_* meshes), coloured via the
    // 'stitching' part through the name fallback in partForMesh.
    ...
    garmentByTarget.forEach((meshes, target) => {
      void createArtworkCanvasTexture(
        ...
        renderHexFor(partColors[target]),
          ...
          // No auto-placed fixed branding on this garment (see drawFixedDsplnMarks).
          const hasFixedLogo = false;

...
function buildSummary(spec: RashguardSerializedState) {
  const frontColor = colorValue(spec.partColors.rightFrontLeg);
  const backColor = colorValue(spec.partColors.rightBackLeg);
  const artworkCount = spec.artworkLayers.length;
  return `Size ${spec.size} / Front ${frontColor} / Back ${backColor} / ${artworkCount} artwork layer${artworkCount === 1 ? '' : 's'}`;
}
...
export function buildRashguardCartLine({
  ...
  return {
    ...
    properties: [
      { name: 'Short Size', value: spec.size },

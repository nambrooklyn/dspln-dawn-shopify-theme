...
import { currentGiProductConfig } from './gi-product-config';
...
function buildSummary(spec: GiSerializedState) {
  ...
  const beltColor = colorValue(spec.belt.color);
  const pantColor = colorValue(spec.pant.colors.body);
  return `Kimono ${spec.kimono.size} / ${kimonoColor} / Belt ${spec.belt.size} ${beltColor} / Pant ${spec.pant.size} / ${pantColor}`;
}
...
export function buildShopifyTestCartLine({
  ...
  const configuratorId =
    designId ?? `${PRODUCT_CONFIG.orderDesignIdPrefix}_${Date.now().toString(36)}`;
...
function cartPropertiesForShopify(line: ShopifyCartLine) {
  return line.properties.reduce<Record<string, string>>((acc, property) => {
    ...
    if (
      property.hidden &&
      !property.name.startsWith('_dspln_artwork_layer_') &&
      !(
        /^https?:\/\//.test(property.value) &&
        /artwork|logo/i.test(property.name)
      ) &&
      ![
        '_dspln_design_id',
        '_dspln_design_url',
    ...
    if (
      property.name.startsWith('_dspln_artwork_layer_') &&
      /^(data:image\/|blob:)/.test(property.value)
    ) {
      return acc;
    }

...
// Map a lining mesh to the garment part whose colour it should echo. The shorts
// ship generic per-leg `Lining` meshes with no side in the name, so echo the
// waistband colour for the faded-inside look.
...
function shouldShowControlsForCamera(
  ...
  if (target === 'rightFrontLeg' || target === 'leftFrontLeg')
    return cameraView === 'front';
  return true; // waistband: visible from both
}
...
async function drawFixedDsplnMarks(
  ...
  // No auto-placed fixed logo on the grappling short — branding is the 3D Label mesh.
}
...
export const RashguardGlbModel = memo(() => {
  ...
  const cloned = useMemo(() => {
    ...
    // No stitch graft here: this model's stitches are already mesh baked into
    // the single GLB (the Topstitch_* meshes), coloured via the 'stitching' part.

...
// Pieces to export, in order. All printable zones take artwork.
...
async function buildPartCanvas(
  ...
  if (opts.showColorFill || !opts.transparentBackground) {
    ...
    // Print the TRUE selected swatch colour — the colour the customer actually
    // picked — not `renderHex`. `renderHex` only exists to compensate for the 3D
    // scene lighting; printing it makes the art file look muted/desaturated vs the
    // configurator. The manufacturer must sublimate the real chosen colour.
...
export async function generateRashguardArtFile(
  ...
  const opts: Required<Omit<ArtFileOptions, 'fileName'>> & { fileName: string } = {
    ...
    fileName: input.options?.fileName ?? 'adult-grappling-short-art-file.pdf',
  };
  ...
  // Pages 1–4: one 8.5×11 render page per view (front, back, left, right).
  if (orderInfo && views) {

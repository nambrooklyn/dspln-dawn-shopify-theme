...
export const GI_PART_PRICES: Record<GiPart, number> = {
  // Standalone kimono base price. Must equal the first rung of this
  // product's Shopify tda_price ladder — the cart matches a design's total
  // against those variants exactly, and an unmatched total falls back to
  // the wrong variant.
  jacket: 55,
  pants: 45,
  belt: 15,
...
export const CAMERA_POSITIONS: Record<CameraView, [number, number, number]> = {
  ...
  'body-close': [0.59, 1.95, 2.6],
  'reinforcement-close': [1.28, 1.45, 1.9],
  'stitching-close': [-0.75, 1.95, 0.9],
...
export const CAMERA_TARGETS: Record<CameraView, [number, number, number]> = {
  ...
  'left-sleeve-close': [0.4, 1.55, 0],
  'right-sleeve-close': [-0.4, 1.55, 0],
  'back-close': [0, 1.7, 0],
  'back-skirt-close': [0, 0.98, 0],
  'left-thigh-close': [0.22, 0.8, 0],
  'right-thigh-close': [-0.22, 0.8, 0],
  'body-close': [0, 1.85, 0],
  'reinforcement-close': [0.1, 1.6, 0],
  'stitching-close': [0.1, 1.7, 0],

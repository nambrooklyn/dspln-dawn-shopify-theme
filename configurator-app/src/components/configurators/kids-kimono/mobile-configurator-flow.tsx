...
export const MobileConfiguratorFlow = memo(
  ({
    ...
    const steps = useMemo<MobileStep[]>(() => {
          ...
          content: (
            <section className="border-border border-b px-3 py-3">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {BELT_COLOR_SWATCHES.map((swatch) => {
                  const isSelected = swatch.name === beltRank;
                  return (
                    <button
                      key={swatch.name}
                      type="button"
                      onClick={() => setBeltRank(swatch.name)}
                      aria-pressed={isSelected}
                      className={`h-10 overflow-hidden rounded-sm border transition-colors ${
                        isSelected
                          ? 'border-2 border-[#17115f] bg-white shadow-sm'
                          : 'border-border bg-background hover:border-[#17115f]/50'
                      }`}
                    ...
                    >
                      <span className="flex h-full w-full flex-col">
                        <span
                          className="block h-2/3"
                          style={{ backgroundColor: swatch.hex }}
                        />
                        <span
                          className="block h-1/3 border-t border-black/10"
                          style={{ backgroundColor: swatch.stripeHex }}
                        />
                      </span>
                    </button>
                  );
      ...
      // Single-item product: only this item's steps, no add/remove step.
      void beltSteps;
      void pantSteps;
      return kimonoSteps.filter((step) => step.key !== 'kimono-add');
    }, [
